...
import type { StaffPersonOption } from "@/lib/staff-person-options";
...
export type StaffPersonSelectProps = {
  value: string;
  options: StaffPersonOption[];
  disabled?: boolean;
...
export function StaffPersonSelect({
  value,
  options,
  disabled = false,
  onChange,
  ...
  return (
    <div ref={rootRef} className={`relative ${className}`}>
      ...
      >
        {selected ? (
          <>
            <PersonColorDot color={selected.color} />
            <span className="min-w-0 flex-1 truncate">{selected.label}</span>
          </>
        ) : (
          <span className="text-zinc-500 dark:text-neutral-400">{placeholder}</span>
        )}
        <ChevronDown
          ...
          {options.map((option) => {
            ...
            return (
              <li key={option.id} role="presentation">
                ...
                >
                  <PersonColorDot color={option.color} />
